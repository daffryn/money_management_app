...
pkgs.mkShell {
  ...
  shellHook = ''
   ...
   trap \
    ...
    if ! psql -U $(whoami) -tAc "SELECT 1 FROM pg_database WHERE datname='money_management'" | grep -q 1; then
      createuser -U $(whoami)
      ...
      psql -U "$PGUSER" -d postgres -c "CREATE DATABASE money_management" || true
    fi
